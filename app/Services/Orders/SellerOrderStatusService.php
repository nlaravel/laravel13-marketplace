...
class SellerOrderStatusService
{
    ...
    {
        return $this->transition($sellerOrder, SellerOrderStatus::PENDING, SellerOrderStatus::CONFIRMED);
    }
    ...
    {
        return $this->transition($sellerOrder, SellerOrderStatus::CONFIRMED, SellerOrderStatus::PROCESSING);
    }

    /**
     * Mark a processing seller order as ready for shipping.
     */
    public function markReadyForShipping(SellerOrder $sellerOrder): SellerOrder
    {
        return $this->transition($sellerOrder, SellerOrderStatus::PROCESSING, SellerOrderStatus::READY_FOR_SHIPPING);
    }
    ...
    {
        return $this->transition($sellerOrder, SellerOrderStatus::READY_FOR_SHIPPING, SellerOrderStatus::SHIPPED);
    }
    ...
    public function markDelivered(SellerOrder $sellerOrder): SellerOrder
    {
        return DB::transaction(function () use ($sellerOrder) {
            ...
            if ($sellerOrder->status !== SellerOrderStatus::SHIPPED) {
                throw new InvalidArgumentException('Only shipped seller orders can be marked as delivered.');
            }
    ...
    public function cancel(SellerOrder $sellerOrder): SellerOrder
    {
        return DB::transaction(function () use ($sellerOrder) {
            ...
            if (in_array($sellerOrder->status, [
                ...
                throw new InvalidArgumentException('This seller order cannot be cancelled.');
            }
    ...
    {
        return $this->transition($sellerOrder, SellerOrderStatus::DELIVERED, SellerOrderStatus::COMPLETED);
    }

    /**
     * Perform a controlled seller order status transition.
     */
    private function transition(SellerOrder $sellerOrder, SellerOrderStatus $from, SellerOrderStatus $to): SellerOrder
    {
        return DB::transaction(function () use ($sellerOrder, $from, $to) {
            $sellerOrder->refresh();
