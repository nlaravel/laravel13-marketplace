    ...
    public function assign(Delivery $delivery, int $deliveryProfileId): Delivery
    {
        return DB::transaction(function () use ($delivery, $deliveryProfileId) {
            ...
            if ($delivery->status !== DeliveryStatus::PENDING) {
                throw new DeliveryException('Only pending deliveries can be assigned.');
            }
    ...
    public function deliver(Delivery $delivery): Delivery
    {
        return DB::transaction(function () use ($delivery) {
            ...
            if ($delivery->status !== DeliveryStatus::IN_TRANSIT) {
                throw new DeliveryException('Only in-transit deliveries can be delivered.');
            }
    ...
    public function fail(Delivery $delivery, ?string $notes = null): Delivery
    {
        return DB::transaction(function () use ($delivery, $notes) {
            ...
            if (in_array($delivery->status, [
                ...
                throw new DeliveryException('Delivered or cancelled deliveries cannot be failed.');
            }
    ...
    public function cancel(Delivery $delivery, ?string $notes = null): Delivery
    {
        return DB::transaction(function () use ($delivery, $notes) {
            ...
            if (in_array($delivery->status, [
                ...
                throw new DeliveryException('Delivered or already cancelled deliveries cannot be cancelled.');
            }
    ...
    private function transition(Delivery $delivery, DeliveryStatus $from, DeliveryStatus $to, array $attributes = []): Delivery
    {
        return DB::transaction(function () use ($delivery, $from, $to, $attributes) {
            ...
            if ($delivery->status !== $from) {
                throw new DeliveryException("Invalid delivery transition from {$delivery->status->value} to {$to->value}.");
            }
